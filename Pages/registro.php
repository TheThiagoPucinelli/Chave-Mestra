...
<body class="bg-gray-100 min-h-screen flex flex-col">
  ...
  <main class="flex-grow container mx-auto px-4 py-10 max-w-7xl">
    <h1 class="text-3xl font-bold text-center text-blue-800 mb-8">Registro de Chaves</h1>

    <input
      type="text"
      id="busca"
      onkeyup="filtrarChaves()"
      placeholder="Buscar por ID, nome, número, descrição, quantidade ou status..."
    ...
    $sql = "
      ...
      GROUP BY c.id_chave
      ORDER BY 
        (CASE WHEN e.id_emprestimo IS NULL THEN 1 ELSE 0 END) ASC, -- Indisponível primeiro
        c.nome ASC
    ";
    ...
    <!-- Tabela para desktop -->
    <div class="hidden md:block overflow-x-auto shadow-lg rounded-lg bg-white border border-gray-200">
    ...
    <?php
    // Resetar ponteiro para reutilizar resultados no modo mobile
    $result->data_seek(0);
    ?>
    ...
    <div class="md:hidden space-y-4">
      <?php while ($row = $result->fetch_assoc()):
        $statusText = ucfirst($row["status_dinamico"]);
        $statusClass = $statusText === 'Disponível'
          ? "text-green-700 bg-green-100 font-semibold px-2 py-1 rounded inline-block"
          : "text-red-700 bg-red-100 font-semibold px-2 py-1 rounded inline-block";
      ...
      ?>
        <div class="bg-white p-4 rounded shadow border border-gray-200">
          <p><strong>ID:</strong> <?= htmlspecialchars($row["id_chave"]) ?></p>
          <p><strong>Nome:</strong> <?= htmlspecialchars($row["nome"]) ?></p>
          <p><strong>Número Identificação:</strong> <?= htmlspecialchars($row["numero_identificacao"]) ?></p>
          <p><strong>Descrição:</strong> <?= htmlspecialchars($row["descricao"]) ?></p>
          <p><strong>Quantidade:</strong> <?= htmlspecialchars($row["quantidade"]) ?></p>
    ...
    <?php else: ?>
      <p class="text-center text-red-500">Nenhuma chave encontrada.</p>
    <?php endif; ?>
